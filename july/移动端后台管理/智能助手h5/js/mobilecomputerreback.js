$.post("http://192.168.1.74:8080/api/kapSecretKey/getDataByKey",{key: "078f83e953444764ad893ae049b1a9c2"},function(data){
	 ...
	 //    heightIframe.style.height = (deviceHeight)+"px";
		...
		desktop_win_json = JSON.parse(desktop_win_json);
		var en=+ desktop_but_json.sideDistanceInput_bstb;
		var end=+ desktop_but_json.belowDistanceInput_bstb;
		//初始化卡通形象还是普通按钮
		console.log(333);
		console.log(desktop_but_json.btnTypeInput_bstb);
		if(desktop_but_json.btnTypeInput_bstb == 1) {
			$(".cartoonItem_bstb").addClass("checked");
			$(".txtItem_bstb").removeClass("checked");
			$(".cartoonButton_bstb").show();
			$(".txtButton_bstb").hide();
			$("#btnTypeInput_bstb").val(1);
					//卡通形象
			$(".cartoonButton_bstb>ul>li").each(function() {
				if($(this).attr("data-value") == desktop_but_json.btnStyleCartoonInput_bstb) {
					console.log(110);
					console.log(desktop_but_json.btnStyleCartoonInput_bstb);
		...
		} else if(desktop_but_json.btnTypeInput_bstb == 0) {
			$(".cartoonItem_bstb").removeClass("checked");
			$(".txtItem_bstb").addClass("checked");
			$(".cartoonButton_bstb").hide();
			$(".txtButton_bstb").show();
			$("#btnTypeInput_bstb").val(0);
		}
		// $("")
		//电话按钮显示
		// console.log(desktop_win_json.form_controlykphonenumshow)
		if(desktop_win_json.form_controlykphonenumshow == 0) {
			$(".ykmobilephonenum").hide();
            $("#img_phonecall").hide();
		} else if(desktop_win_json.form_controlykphonenumshow == 1) {
			$(".ykmobilephonenum").show();
            $("#img_phonecall").show();
		}
		// 窗体的头部按钮的颜色
		$("#smallWindowHeader").css("background",desktop_win_json.wheel_demo);
		$("#windowSendBtn").css("background",desktop_win_json.wheel_demo);
		$("#wheel-demo").val(desktop_win_json.wheel_demo);
		console.log(desktop_win_json.wheel_demo)
		$("#minicolors-swatch-color1").css({"background":desktop_win_json.wheel_demo})
	    $(".windowColor>ul>li").each(function() {
	    	if($(this).attr("data-value") ==desktop_win_json.wheel_demo.toLowerCase()) {
	    		$(this).siblings().removeClass("checked");
	    		$(this).addClass("checked");	
	    	}
	    });
	    ...
	    $("#btnTitleInput_bstb").val(desktop_but_json.btnTitleInput_bstb);
	    //文字按钮样式
	    $(".txtButton_bstb>ul>li").each(function() {
	    	if($(this).attr("data-value") == desktop_but_json.btnStyleInput_bstb) {
	    		$(this).siblings().removeClass("checked");
	    		$(this).addClass("checked");
	    	}
	    });
	    ...
	    $("#diy-color").val(desktop_but_json.diy_color)
	    $("#minicolors-swatch-color").css({backgroundColor:desktop_but_json.diy_color});
	    $(".btnColor_bstb>ul>li").each(function() {
	    	if($(this).attr("data-value") == desktop_but_json.diy_color.toLowerCase()) {
	    		$(this).siblings().removeClass("checked");
	    		$(this).addClass("checked");	
	    	}
	    });
	    ...
	    $("#smallWindowHeader>span").text(desktop_win_json.tenantSiteName)
	    // 按钮的左右位置
	    $("#btnPositionInput_bstb").val(desktop_but_json.btnPositionInput_bstb);
	    $(".btnPosition_bstb>ul>li").each(function() {
	    	if($(this).attr("data-value") == desktop_but_json.btnPositionInput_bstb) {
	    		$(this).siblings().removeClass("checked");
	    		$(this).addClass("checked");
	    	}
	    });
	    ...
	    // 按钮左右偏移位置
	    $("#sideDistanceInput_bstb").val(desktop_but_json.sideDistanceInput_bstb);
	    if (desktop_but_json.belowDistanceInput_bstb == 0) {
            $(".ykmobile-fa").css({left: sideDistanceInput_bstb + 'px'});
        } else if(desktop_but_json.belowDistanceInput_bstb == 1) {
            $(".ykmobile-fa").css({right: sideDistanceInput_bstb+ 'px'});
        };
	     // 按钮上下偏移位置
	    $("#belowDistanceInput_bstb").val(desktop_but_json.belowDistanceInput_bstb);
	   	$(".ykmobile-fa").css({bottom: belowDistanceInput_bstb + 'px'});
		if(desktop_but_json.btnTypeInput_bstb==0){//0为按钮，1为窗体
			  $(".wzh_29_box_jqr").css({width:'125px',height:'50px'});
			  $("#background").hide();
			  // 按钮样式
			$("#btnTypeInput_bstb").val(desktop_but_json.btnStyleInput_bstb)
			if (desktop_but_json.btnStyleInput_bstb == 0) {
	            $(".huilan-aikf-show").css({
	                width: "100%",
	                height: "50px",
		 ...
		 // 按钮颜色
		  ...
		  if(desktop_but_json.btnPositionInput_bstb==0){
		      ...
		      // 按钮上下偏移
		      $(".ykmobile-fa").css({bottom: (10+end) + 'px'})
		  }else if(desktop_but_json.btnPositionInput_bstb==1){
	            $(".ykmobile-fa").css({
	                left: "auto",
	                right: "0px",
